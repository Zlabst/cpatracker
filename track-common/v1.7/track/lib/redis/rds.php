...
class rds {
	...
	private function _exists($k,$func=''){
		...
		if(!$this->_conn()->exists($k)){
			if(empty($func) or $this->exists($k.'_null')) return false;
			else return $this->_calc($k,$func);
		}
		return true;
	}
	...
	public function sget($k,$func=''){
		if(!$this->_exists($k,$func)) return array();
	...
	public function in($k,$v,$func=''){
		if(!$this->_exists($k,$func)) return false;
		return $this->_conn()->sismember($k,$v)?1:0;
	}
	
	public function cnt($k,$func=''){
		if(!$this->_exists($k,$func)) return 0;
	...
	public function dec($k,$v=1){
		if($v==1) return $this->_conn()->decr($k);
		return $this->_conn()->decrby($k,$v);
	}
	
	public function inc($k,$v=1){
		if($v==1) return $this->_conn()->incr($k);
	...
	public function sinter($k1,$k2,$f1='',$f2=''){
		if(!$this->_exists($k1,$f1)) return false;
		if(!$this->_exists($k2,$f2)) return false;
	...
	public function __call($method, $args) {
		...
		} else throw new InvalidArgumentException("Method {$method} doesn't exist");
		/*
		print_r($method);
