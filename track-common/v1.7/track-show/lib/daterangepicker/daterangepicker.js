...
!function ($) {
    ...
    DateRangePicker.prototype = {
        ...
        clickApply: function (e) {
            this.updateInputText();
            this.hide();
            //submit form when range selected
            var str = $('#putdate_range').val();
            if(str){
                var from = str.substring(10,-1);
                var to = str.substring(13);
                $('#form_range_from').val(from.substring(6)+'-'+from.substring(5, 3)+'-'+from.substring(2, -1));
                $('#form_range_to').val(to.substring(6)+'-'+to.substring(5, 3)+'-'+to.substring(2, -1));        
                $('[name=datachangeform]').submit()
            }
        },
