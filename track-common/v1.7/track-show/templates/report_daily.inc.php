...
foreach ($var['arr_report_data'] as $source_name => $data) {
    ...
    if (($var['report_params']['mode'] == '' and $data['out'] > 0) or ($var['report_params']['mode'] == 'lp' and $data['out'] == 0))
        continue;
...
if ($var['report_params']['mode'] != 'popular') {
    echo '<tfoot><tr><td colspan="2">Итого</td>';
    foreach ($var['arr_dates'] as $cur_date) {
        echo '<td>' . get_clicks_report_element2($column_total_data[$cur_date], false, false) . '</td>';
    }
    echo '<td>' . get_clicks_report_element2($table_total_data, false, false) . '</td>';
    echo "</tr></tfoot>";
}
