...
<div class="table-report-box">
    <table class="table table-striped table-report dataTableT<?php echo $table_n; ?> ">
        <thead>
            <tr><th><?php if ($var['report_params']['group_by'] == '') { ?>Название</th><th>Значение<?php } else {
    echo $group_types[$var['report_params']['group_by']][0];
} ?></th><th>Переходы</th><th>От&nbsp;общего</th><?php if ($var['report_params']['conv'] != 'none') { ?><th class="col_s">Продажи</th><th class="col_l">Лиды</th><th class="col_a">Действия</th><th class="col_s">Конверсия</th><th class="col_l">Конверсия</th><th class="col_a">Конверсия</th><?php } ?><th>Затраты</th><?php if ($var['report_params']['conv'] != 'none') { ?><th class="col_s col_a">Прибыль</th><th>EPC</th><th class="col_s col_a">ROI</th><th class="col_s">CPS</th><th class="col_l">CPL</th><th class="col_a">CPA</th><?php } ?></tr>
        </thead>
        <tbody>
            <?php
            ...
            foreach ($var['arr_report_data'] as $k => $r) {
                ...
                if ($var['report_params']['group_by'] == '') {
                    ...
                    if (in_array($var['report_params']['conv'], array('act', 'sale', 'lead')) and $r[$var['report_params']['conv']] == 0)
                        continue;
