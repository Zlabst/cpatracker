...
function glink($v, $li = false, $name = '') {
	...
	if(array_key_exists($v, $params['filter'][0]) or ($v == 'campaign_name' and array_key_exists('ads_name', $params['filter'][0]))) return ''; 
...
<div class='col-md-12'>
	<div class="btn-group">
		...
		<?php
			...
			for($i = 1; $i <= 5; $i++) {
				if(!$var['campaign_params'][$i]) continue; // есть ли в наших данных переходы с таким параметром
			...
			for($i = 1; $i <= 15; $i++) {
				if(!$var['click_params'][$i]) continue; // есть ли в наших данных переходы с таким параметром
